...
def plot_barv_stacked(labels, data, colors, title='', ylabel='', save='',
    ...
    fig, ax = plt.subplots(figsize=params["figs_comp_st"],
                           dpi=params["dpi"])
...
def plot_histogram(data, save_path, bins=None, cumulative=False,
                   ...
                   xaxis=True, xticks=None, cm=None, params=None):
    """
    Plot a histogram
    ...
    fig, ax = plt.subplots(figsize=params["figs_station_usage_hist"],
                           dpi=params["dpi"])
    if xlim is not None:
