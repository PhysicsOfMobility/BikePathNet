...
def plot_used_nodes(city, save, G, trip_nbrs, stations, plot_folder, params=None):
    ...
    ns = [params["ns_station_usage"] if n in stations else 0 for n in G.nodes()]
    ...
    cmap = (
        ["#808080"]
        + [rgb2hex(cmap(n)) for n in reversed(np.linspace(1, 0, max_n, endpoint=True))]
        + ["#ffffff"]
    )
    ...
    if min_n <= 0.1 * max_n:
        r = magnitude(max_n)
        cbar = fig2.colorbar(
            sm, orientation="horizontal", cax=cbaxes, ticks=[0, round(max_n / 2), max_n]
        )
        cbar.ax.set_xticklabels(
            [0, int(round(max_n / 2, -(r - 2))), round(max_n, -(r - 1))]
        )
    else:
    ...
    if params["titles"]:
        fig2.suptitle(
            f"{city.capitalize()}, Stations: {station_count}, " f"Trips: {trip_count}",
            fontsize=params["fs_title"],
        )

    fig2.savefig(
        f'{plot_folder}{save}_stations_used.{params["plot_format"]}',
        bbox_inches="tight",
    )
...
def plot_bp_comparison(
    ...
    plt.savefig(
        f'{plot_folder}{save}_bp_build_{mode}.{params["plot_format"]}',
        bbox_inches="tight",
    )
    plt.close(fig)
...
def plot_mode(
    ...
    trdt_st_ps = {
        st: len_on_st
        for st, len_on_st in trdt_ps.items()
    ...
    ax1.plot(bpp_ps, ba_ps, c=params["c_ba"], ms=params["ms_ba"], marker=params["m_ba"])
    xmax, ymax = coord_transf(
        bpp_ps, max([ba_y, ba_ps]), xmax=1, xmin=0, ymax=1, ymin=0
    )
    ax1.axvline(
        x=bpp_ps,
        ymax=ymax,
    ...
    xmax_10, ymax_10 = coord_transf(bpp_10, ba_10, xmax=1, xmin=0, ymax=1, ymin=0)
    ax1.axvline(
        x=bpp_10,
        ymax=ymax_10,
        ymin=0,
    ...
    ax1.set_ylabel(
        r"bikeability $b(\lambda)$", fontsize=params["fs_axl"], color=params["c_ba"]
    )
    ...
    ax12.plot(
        bpp_cut,
        [x / total_cost[end] for x in total_cost[: end + 1]],
        c=params["c_cost"],
        label="total cost",
        lw=params["lw_cost"],
    )
    ...
    xmin, ymax = coord_transf(bpp_ps, cost_ps, xmax=1, xmin=0, ymax=1, ymin=0)
    ax1.axvline(
        x=bpp_ps,
        ymax=ymax,
        ymin=0,
    ...
    if params["legends"]:
        ax1.legend(handles=handles, loc="lower right", fontsize=params["fs_legend"])

    fig1.savefig(
        f'{plot_folder}{save}_ba_tc.{params["plot_format"]}', bbox_inches="tight"
    )

    comp_st_driven = {
        st: [len_on_st[bpp_idx], trdt_st_ps[st]] for st, len_on_st in trdt_st.items()
    }
    plot_barv_stacked(
        ["Algorithm", "P+S"],
        comp_st_driven,
    ...
    for bp_mode in ["algo", "p+s", "diff"]:
        plot_bp_comparison(
            ...
            save=save,
            G=nxG_plot,
            ee_algo=edited_edges_nx,
            ee_cs=bike_paths_ps,
            bpp_algo=bike_path_perc,
            bpp_cs=bike_path_perc_ps,
            stations=stations,
...
def plot_city(city, save, paths=None, params=None):
    ...
    print(f"{save}: area={area:.0f}km^2, stations={len(stations)}, " f"trips={trips}")
    ...
    ps_bike_paths = [
        e
        for e in nxG_calc.edges()
        if get_street_type_cleaned(nxG_calc, e, multi=False) in ["primary", "secondary"]
    ]
    data_ps = calc_single_state(
        nxG=nxG_calc, trip_nbrs=trip_nbrs, bike_paths=ps_bike_paths, params=params
    )
    ...
    bpp_ps, ba_ps, cost_ps, = plot_mode(
        city=city,
        save=save,
        data=data,
        data_ps=data_ps,
...
def plot_comp_hom_demand(city, save, bpp_ed, bpp_hom, ba_ed, ba_hom, params, paths):
    """
    ...
    ax.plot(bpp_ed, ba_ed, c=params["c_ed"], lw=params["lw_ed"], label="emp. demand")
    ax.plot(
        bpp_hom, ba_hom, c=params["c_hom"], lw=params["lw_hom"], label="hom. demand"
    )
    poly = ax.fill(
        np.append(bpp_ed, bpp_hom[::-1]),
    ...
    fig.savefig(
        f'{paths["plot_folder"]}results/{save}/'
        f'{save}_ba_hom_comp.{params["plot_format"]}',
        bbox_inches="tight",
    )


def plot_city_hom_demand(
    city, save, paths, params, hom_pattern="hom", nbr_of_hom_sets=10
):
    """
    ...
    bpp_hom, ba_hom = average_hom_demand(
        save=save,
