...
def core_algorithm(nkG, edge_dict, trips_dict, nk2nx_edges, street_cost,
                   starttime, logpath, output_folder, save,
                   dynamic, penalty):
    """
...
def run_simulation(city, save, params=None, paths=None):
    ...
    # Initial Log
    log_to_file(logpath, f'Started {save}', start=sd, stamp=False,
                difference=False)
    ...
    hf.close()
    # Calculate data
    core_algorithm(nkG=nkG, edge_dict=edge_dict,
                   trips_dict=trips_dict, nk2nx_edges=nk2nx_edges,
                   street_cost=street_cost, starttime=starttime,
                   logpath=logpath, output_folder=output_folder,
                   save=save,
                   penalty=params["penalty weighting"],
                   dynamic=params["dynamic routes"])

    # Print computation time to console and write it to the log.
    log_to_file(logpath, f'Finished {save}.',
                stamptime=time.localtime(), start=starttime,
                end=time.time(), stamp=True, difference=True)
    return 0
